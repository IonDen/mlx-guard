...
summary of each page, and writes the summaries to a JSON file. Seventy pages, about a second each.
...
summaries[index] = generate(model, tokenizer, prompt, max_tokens=args.max_tokens, prompt_cache=cache)
if args.keep_caches:
    caches[index] = cache
```

A prompt cache for this model costs about 115 KB per token. A 3,200-character page is roughly 800
tokens. So every page pins about 90 MB that is never released. The script prints what MLX thinks
it holds after each page, so the growth is visible if you look. Most people do not look, because
the job is producing summaries and the fan is quiet.
...
corpus in this tutorial is 70 pages, so this exact run would have finished. Feed the same script a
real document dump, two thousand pages, and it wants 180 GB of caches.

What happens then is not a clean error. MLX does not know the machine is full; unified memory
means the GPU's allocations are the system's memory, and there is no separate pool to run out of.
macOS compresses pages first, then swaps, and each of those makes every allocation slower, so the
job that was doing a page a second now does a page every ten seconds, then every minute, while the
memory keeps growing. The pointer stops moving. Applications stop redrawing. If you are lucky,
`kernel_task` or a jetsam decision kills something, possibly your terminal, possibly the browser
with the form you were filling in. If you are unlucky, the machine reboots, and the fine print in
the [threat model](docs/THREAT_MODEL.md) explains that on macOS 26.4 and later there is also a
driver bug that can panic the kernel under Metal load with the footprint nowhere near full, which
no tool in user space can prevent.
...
page 1/12 in 1.7s; mlx active 1.79 GiB; caches held 1
page 2/12 in 1.0s; mlx active 1.90 GiB; caches held 2
page 3/12 in 0.7s; mlx active 1.96 GiB; caches held 3
page 4/12 in 1.2s; mlx active 2.07 GiB; caches held 4
page 5/12 in 0.5s; mlx active 2.09 GiB; caches held 5
page 6/12 in 1.0s; mlx active 2.18 GiB; caches held 6
page 7/12 in 1.0s; mlx active 2.26 GiB; caches held 7
page 8/12 in 1.0s; mlx active 2.34 GiB; caches held 8
...
done: 12 summaries in 12s -> reports/observe-progress.json
mlx-guard: child_exited at 14259ms; 252 samples, 0 signals
$ echo $?
0
```
...
"calibration": {
  ...
  "observed_duration_ms": 14201,
  "peak_aggregate_footprint_bytes": { "status": "available", "value": 3822193016 },
  "peak_growth_bytes_per_second": { "status": "available", "value": 14426965071 },
  "automatic_limit_bytes": null,
  "guidance": "choose_explicit_limit_from_repeated_representative_runs"
}
```

Two things to read here. The peak is 3.8 GB, and that is the process as the operating system
charges it, not the number MLX prints, which is why it sits about a gigabyte above the script's
own figure. The peak growth rate looks alarming at 14 GB/s until you realize it is the model
loading in the first two seconds; the leak's own pace is in the samples, and it is the 90 MB per
page you can also see above. The guard does not turn any of this into a limit for you.
`automatic_limit_bytes` is null and always will be, because a number invented by the tool would
be a number nobody chose.

So we chose one. On the machine that is going to run this job for real, say a 16 GB laptop with a
browser and an editor open, 6 GiB is what this process may have. That is comfortably above the
3.8 GB peak of a 12-page run, and it is a number the rest of the laptop can live with.
...
70 pages, 70 to do, keep_caches=True
page 1/70 in 1.2s; mlx active 1.79 GiB; caches held 1
...
page 37/70 in 0.9s; mlx active 4.88 GiB; caches held 37
page 38/70 in 1.0s; mlx active 4.96 GiB; caches held 38
page 39/70 in 0.8s; mlx active 5.05 GiB; caches held 39
mlx-guard: policy_intervention at 39927ms; 699 samples, 1 signal
$ echo $?
75
...
ceiling about ten percent above it at which it stops asking and kills. Then 39 pages of ordinary
...
The report says what happened at the resolution the transcript cannot. The process crossed into
the warning band at 34.2 s. At 39.87 s two consecutive samples were at or above the limit, the
guard sent `SIGTERM` to the whole process group, and at 39.93 s the group was gone:

```json
"outcome": {
  "at_ms": 39927,
  "kind": "policy_intervention",
  "final_footprint_bytes": { "status": "available", "value": 6473550176 },
  "child_status": { "status": "signaled", "signal": 15 }
},
"signals": [
  { "at_ms": 39868, "signal": 15, "target": "owned_process_group", "result": "delivered", "reason": "footprint" }
],
"checkpoint": { "status": "not_negotiated", "reason": "footprint" }
```

What that bought is easy to state. The Mac did not notice. The other applications kept their memory, the
pointer kept moving, and the run ended forty seconds in with a file that says exactly why. Compare
that with the arithmetic section: a rebooted machine and a guess.

There is also a bill. Thirty-nine summaries were computed and none were saved, because the
script only writes its output at the end and `SIGTERM` arrived before the end. The `ls` at the
bottom of the transcript is the whole problem in one line. A limit turns a catastrophe into a
loss; it does not by itself turn a loss into progress.
...
```python
def connect_checkpoint(progress, summaries):
    try:
        import mlx_guard
    except ImportError:
        return None
...
unchanged on its own. Inside the loop the script calls `worker.poll()` once per page, which is
where the request is delivered; the callback writes the progress file and reports the file's
size, and never a path. The guard gives it one second by default, which is a long time to write a
12 KB file and a short time to save a model, which is why the progress file is written
incrementally and the callback only has to finish it.

With `--resume`, the script skips pages that are already in the progress file. Put the two
together in a loop that restarts only on exit 75, and the batch finishes itself:
...
page 1/70 in 1.2s; mlx active 1.79 GiB; caches held 1
...
page 40/70 in 1.1s; mlx active 5.16 GiB; caches held 40
checkpoint 12638027534934235804: saved 40 summaries
mlx-guard: policy_intervention at 40677ms; 755 samples, 2 signals
mlx-guard: enforcing a 6442450944-byte footprint limit; emergency KILL at 7086696038 bytes, about 10% above the limit
70 pages, 30 to do, keep_caches=True
page 41/70 in 1.2s; mlx active 1.79 GiB; caches held 1
...
page 70/70 in 1.1s; mlx active 4.58 GiB; caches held 30
done: 70 summaries in 31s -> reports/progress.json
mlx-guard: child_exited at 32952ms; 612 samples, 0 signals
...
summaries, and `SIGTERM` followed. The second run started with thirty pages to do and finished
them. The first report records the whole exchange, including the request id the script printed,
so a later process can prove which run wrote which file:

```json
"checkpoint": {
  "status": "acknowledged_unverified_durability",
  "at_ms": 40612,
  "request_id": 12638027534934235804,
  "reason": "footprint",
  "artifact": { "kind": "file", "size_bytes": 12030 }
}
```

The status is honest about what the guard knows. The script said it completed the save; the
guard did not open the file to check. Durability is the worker's promise, and the report says so.

The leak is still there. The job still cannot hold more than about forty pages of caches inside
6 GiB. But the seventy summaries are in `reports/progress.json`, the machine was never in
...
mlx-guard: enforcing a 6442450944-byte footprint limit; emergency KILL at 7086696038 bytes, about 10% above the limit
70 pages, 70 to do, keep_caches=False
page 1/70 in 1.2s; mlx active 1.79 GiB; caches held 0
...
page 35/70 in 0.9s; mlx active 1.77 GiB; caches held 0
...
page 70/70 in 1.1s; mlx active 1.79 GiB; caches held 0
done: 70 summaries in 69s -> reports/fixed-progress.json
mlx-guard: child_exited at 70983ms; 1320 samples, 0 signals
$ echo $?
0
```

Flat at 1.8 GiB from page one to page seventy. The report's highest footprint is 2.84 GiB, half
the limit, and the exit code is the script's own. This is the run you keep the guard on for
anyway: a limit that never fires costs a few percent of one CPU core and gives you a report that
says the job stayed where you expected, which is the evidence you will want the day the next
Tuesday-afternoon change lands.
...
lines on a checkpoint callback and a resume flag; a loop on exit 75 then finishes the work
without you. And when the report shows a flat line, keep the guard on, because the report is the
proof.
