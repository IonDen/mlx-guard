...
CLI retry with the same report path then exited 74. From 0.3.0 the supervised command reads `/dev/null`
...
two-hour soak, and the escalation envelope. Those measurements, the tutorial, and the in-house trial
were recorded on macOS 26.6.2. The same Mac has since moved to macOS 27.0, where the `mlx-train-perf`
proof was recorded and where this release's full test suite and wheel proof passed; the bounds were
not re-measured there. The release workflow must also pass on GitHub's `macos-15` arm64 runner
