...
| **verified, shared VM** | The same, on a hosted virtual machine with uncontrolled co-tenancy. The full macOS test suite passes there on every release, and its escalation-envelope captures corroborate the reference host's; its timings are never a reference. |
...
| Apple M1 Max | 32 GB | verified | [0.2.0 reference bundle](../evidence/v0.2.0/m1-max-32gb/README.md): MacBook Pro (MacBookPro18,2), 10 cores, macOS 26.6.2 (25G83), commit `59ea303`, captured 2026-09-11; the [soak bundle](../evidence/v0.2.0/m1-max-32gb/soak/README.md) on the same host |
| Apple M1 Ultra | none | untested | — |
| Apple M1 (Virtual), GitHub `macos-15` runner | 7 GB, 3 vCPU | verified, shared VM | Full macOS suite on every CI run; [pooled escalation-envelope captures](../evidence/v0.2.0/github-macos-15-shared/README.md): workflow run 34591393999, commit `9d42fc8`, captured 2026-09-11 |
...
Some failures happen with the supervised process's footprint well inside any limit, so no
...
|---|---|---|
| IOGPU driver bug under Metal workloads, macOS 26.4 through 26.6 | Kernel panic in Apple's IOGPU kernel extension: `IOGPUMemory.cpp:550 completeMemory() prepare count underflow`, also `IOGPUGroupMemory.cpp:219` | Reported independently against MLX inference ([mlx #3346](https://github.com/ml-explore/mlx/issues/3346), [mlx #3186](https://github.com/ml-explore/mlx/issues/3186)), distributed inference ([exo #1972](https://github.com/exo-explore/exo/issues/1972)), and a serving stack ([oMLX #557](https://github.com/jundot/omlx/issues/557)) on M1 Max and M3 Ultra hosts; reporters state the only mitigation is not using the GPU. Unfixed as of late August 2026. An external supervisor cannot reach it. |
...
The bundle's `provenance.json` carries five hardware lines (model name, model identifier, chip,
core count, memory) and the macOS, kernel, and toolchain versions; the profile label, for example
`m3-pro-36gb`, is derived from the chip and memory lines. Before anything is published the script
scans every file for serial numbers, UUIDs, and local paths and refuses to publish if it finds
one. The cargo transcripts land in a sibling `.logs` directory that stays with you.
...
issue with the published directory attached as one zip. The cell is then published as
community-measured with the provenance from the bundle. The reference host's own cells use the
