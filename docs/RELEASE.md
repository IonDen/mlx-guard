...
- Soak gate: run `scripts/soak-reference-host.sh <out>` on the M1 Max, with `<out>` outside the
  repository, whenever supervision, sampling, identity-tracking, or intervention code changed since
  the last release. Copy the JSON files it publishes into `evidence/vX.Y.Z/m1-max-32gb/soak/` and
  write the README that records the measured supervisor footprint, CPU, and sample-window numbers;
  the cargo transcripts and the raw report land in the sibling `<out>.logs` directory and are not
  committed. The four chunks run 30 minutes each, a chunk counts only once it passed, and the
  script resumes a run that was interrupted, so a bundle costs about two hours of an otherwise idle
  machine. The soak tests stay `#[ignore]`d in CI.
