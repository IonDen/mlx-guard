...
launch, so the runner decides by call site: a discovery error raised by `start()` permits the
direct launch, and the same error type raised later by `GuardProcess.wait()` or
`GuardProcess.poll()` does not, because the final report read checks the binary version again
after the condition has already run. The
...
[`mlx-train-perf` evidence bundle](../../evidence/v0.2.0/mlx-train-perf/README.md), recorded on the
0.2.0 wheel against `mlx-train-perf` 0.8.0. The
[0.1.0-era bundle](../../evidence/v0.1.0/mlx-train-perf/README.md) stays as the record of the first
adapter.
