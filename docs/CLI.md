...
Exit 70 outranks an intervention: if observation was lost or a signal could not be delivered, the
result is 70 even though TERM or KILL may have been sent — check the report's signals. Otherwise,
once the policy requests a checkpoint or sends TERM, or reaches KILL through its own deadline, 75
owns the result even if the command ends on its own; a command ended by a forwarded terminal signal
keeps `128+n`. When the root exits while owned-group members survive, `run` terminates them (TERM,
one-second grace, KILL) and reports the root's own status unless KILL was needed; `observe` ends at
root exit, leaves survivors running, and reports it.
