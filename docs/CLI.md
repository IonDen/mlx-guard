...
an interactive terminal, the command receives `/dev/null` instead and the supervisor prints one line
on stderr: `mlx-guard: standard input is a terminal, so the command reads from /dev/null instead`. A
